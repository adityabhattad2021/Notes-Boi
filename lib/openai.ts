...
const config = new Configuration({
    apiKey: process.env.OPENAI_KPI_KEY
});

const openAI = new OpenAIApi(config);


export async function generateImagePrompt(name: string):Promise<string|undefined> {
    try {
        ...
        console.log("[GENERATE_IMAGE_PROMPT]: ", error);
    }
}


export async function generateImage(image_description:string):Promise<string|undefined> {
    try{
        ...
        console.log("[GENERATE_IMAGES]: ",error);
    }
}